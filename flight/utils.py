...
from  destination.models import Week, Place, Flight
...
def addPlaces():
    file = open("./sourceData/airports.csv", "r")
    print("Adding Airports...")
    total = get_number_of_lines("./sourceData/airport_cities.csv")
...
def addDomesticFlights():
    file = open("./sourceData/domestic_flights.csv", "r")
    print("Adding Domestic Flights...")
    total = get_number_of_lines("./sourceData/domestic_flights.csv")
    for i, line in tqdm(enumerate(file), total=total):
        if i == 0:
            continue
        data = line.split(',')
...
def addInternationalFlights():
    file = open("./sourceData/international_flights.csv", "r")
    print("Adding International Flights...")
    total = get_number_of_lines("./sourceData/international_flights.csv")
    for i, line in tqdm(enumerate(file), total=total):
        if i == 0:
            continue
        data = line.split(',')
